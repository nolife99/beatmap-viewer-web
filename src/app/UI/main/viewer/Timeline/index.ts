...
import type TimelineHitObject from '../../../../BeatmapSet/Beatmap/Timeline/TimelineHitObject.ts';
...
export default class Timeline {
	...
	private buildRuler() {
		...
		for (let ti = 0; ti < this._timingPoints.length; ti++) {
			...
			const sectionEnd =
				ti + 1 < this._timingPoints.length
			...
			while (t <= sectionEnd) {
				const isWholeBeat =
					Math.round(
						t - (Math.round((t - startTime) / beatLength) * beatLength + startTime)
					) === 0;
				const isDominant =
					isWholeBeat &&
				...
				if (!isWholeBeat) {
					...
					const idx = Math.round((t - nearestWholeBeat) / (beatLength / divisor));
