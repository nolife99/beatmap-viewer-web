...
export default class Gameplays extends ScopedClass {
	...
	reLayoutChildren(target?: Gameplay) {
		...
		const missingLast = deserialized.length % columnsCount === 0
			? 0
		...
		for (let i = 0; i < deserialized.length; i++) {
			...
			const newLayout = {
				...
				left: overlapGameplays ? 0 : (i % columnsCount) * w +
				...
				paddingRight: w !== 100
					? col === columnsCount - 1 || i === deserialized.length - 1 ? 10 : 5
					: 0,
				scale: 1,
				...
				.onUpdate(
					({
						...
						gameplay.container.layout = {
							...
							left: gameplay === target
								? `${newLayout.left}%`
								: `${left ?? 0}%`,
							width: gameplay === target
								? `${newLayout.width}%`
								: `${width ?? 0}%`,
							height: gameplay === target
								? `${newLayout.height}%`
								: `${height ?? 0}%`,
							paddingTop,
							paddingLeft,
