import MirrorConfig from '../Config/MirrorConfig.ts';
...
async function getBeatmapsetId(beatmapId: string) {
	try {
		...
		const res = await fetch(`https://api.try-z.net/beatmaps?ids=${beatmapId}`);
		if (!res.ok) throw new Error(`HTTP ${res.status}`);
		const { beatmaps }: { beatmaps: BeatmapData[] } = await res.json();
...
async function getBeatmapsetIdFromHash(hash: string) {
	try {
		...
		inject<Loading>('ui/loading')?.setText('Getting beatmapsetId...');
		const res = await fetch(`https://api.try-z.net/b/h/${hash}`);
		if (!res.ok) throw new Error(`HTTP ${res.status}`);
		const beatmap: BeatmapData = await res.json();
...
const fetchBlobFromMirror = async (
	...
	try {
		...
		const blob = await fetchBlobWithProgress(
			...
			(pct) => inject<Loading>('ui/loading')?.setText(
				retry === 0
					? `Downloading with ${selectedMirror.name}: ${(100 * pct).toFixed(2)}%`
					: `Retrying with ${selectedMirror.name}: ${(100 * pct).toFixed(2)}%`
			)
		);
...
export async function getBeatmapFromExternalUrl(url: string) {
	try {
		...
		const blob = await fetchBlobWithProgress(
			...
			(pct) => inject<Loading>('ui/loading')?.setText(`Downloading map: ${(100 * pct).toFixed(2)}%`)
		);
