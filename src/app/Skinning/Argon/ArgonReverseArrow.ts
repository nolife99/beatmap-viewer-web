import DrawableSliderRepeat from '../../BeatmapSet/Beatmap/HitObjects/DrawableSliderRepeat.ts';
...
export const update = (drawable: DrawableSliderRepeat, time: number) => {
	...
	if (loopCurrentTime < moveOutDuration) {
		...
		drawable.ringPiece.x = Easings.Out(loopCurrentTime / moveOutDuration) *
			moveDistance;
	} else {
		drawable.reverseArrow.scale.set(
			1.3 -
		...
		drawable.ringPiece.x = moveDistance -
			Easings.Out(
				(loopCurrentTime - moveOutDuration) /
				(moveOutDuration + moveInDuration)
			) *
